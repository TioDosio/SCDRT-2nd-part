...
void communication::acknowledge_loop(Node *node)
{
  while (can0.checkReceive() && !isMissingAckEmpty())
  {
    ...
    {
      if (previous_ack(canMsgRx))
      {
        printf("E mandei mesmo de volta %c %c\n", last_ack.data[1], last_ack.data[2]);
        err = can0.sendMessage(&last_ack);
      }
      else if (canMsgRx.can_id == my_desk->getDeskNumber() || canMsgRx.can_id == 0) // Check if the message is for this desk (0 is for all the desks)
      {
    ...
    {
      // Serial.printf("Antes tinha isto:\n");
      ...
      // Serial.printf("Ack received from %d -> %c %c\n", canMsgRx.can_id, canMsgRx.data[1], canMsgRx.data[2]);
      confirm_msg(canMsgRx);

      // Serial.printf("Depois fiquei com isto:\n");
      // for (const int &element : missing_ack)
      // {
      //   desks_connected.erase(element);
      //   Serial.printf("%d ", element);
...
void communication::resend_last_msg()
{
  ...
  if (last_msg_counter == 5) // After 5 tries, remove the nodes that didn't ack
  {
    ...
    for (const int &element : missing_ack)
    {
      ...
      desks_connected.erase(element);
      missing_ack.erase(element);
    }
  }
  Serial.printf("Resending message %c %c %c to %d\n", last_msg_sent.data[0], last_msg_sent.data[1], last_msg_sent.data[2], last_msg_sent.can_id);
 ...
 */
void communication::start_consensus(int dest)
{
  struct can_frame canMsgTx;
  canMsgTx.can_id = dest;
...
bool communication::previous_ack(can_frame canMsgRx)
{
  for (int i = 0; i < 6; i++)
  {
    if (canMsgRx.data[i] != last_ack.data[i + 1])
    {
      return false;
    }
  }
